...
export class WasmMemoryInterface {
  ...
  loadU16Array(addr, len) {
    return new Uint16Array(this.memory.buffer, addr, len);
  }
  ...
  loadI8Array(addr, len) {
    return new Int8Array(this.memory.buffer, addr, len);
  }
...
export function odinSetupDefaultImports(
  wasmMemoryInterface,
  consoleElement,
  memory,
) {
...
export async function runWasm(
  ...
  let imports = odinSetupDefaultImports(
    wasmMemoryInterface,
    consoleElement,
  ...
  if (exports.memory) {
    if (wasmMemoryInterface.memory) {
      console.warn(
        'WASM module exports memory, but `runWasm` was given an interface with existing memory too. Did you mean to use `-extra-linker-flags:"--import-memory"` to tell the compiler not to export memory?',
      );
    }
