...
import { Logger, LogLevel } from "../src/core/logging/Logger.js";
...
export const commands = new DCommandRegister();
commands.addCommand(
  new DCommand(
    "list",
    () => {
    ...
    if (command) {
      ...
      socket.send(
        new DMessage()
          ...
          .setData({ result: res, command: commandName, description: command.description })
          .toString(),
      );
    } else {
      socket.send(
